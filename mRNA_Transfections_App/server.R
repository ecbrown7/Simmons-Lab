...
shinyServer(function(input, output) {
    ...
    output$info <- renderTable({
        ...
        #Create function to return mRNA info table
        mRNA_transfections2 = function(volTf, numWells, delWell, porPerc, mmRNARatio, messMaxDil) {
            ...
            cypconc = c(input$bgal, input$a, input$aa, input$b, input$c, input$cc, input$ccc, input$d, input$e, input$j, input$aaa)
            ...
            for(i in 1:nrow(cyps)){
                ...
                optiVol2 = ifelse(finalPor < 0.05, adjVol - mRNAvol - mmVol - finalPor50, 
                                  ifelse(finalPor < 0.1, adjVol - mRNAvol - mmVol - finalPor20,
                ...
                output = c(cyps$cyp[i], round(as.numeric(ifelse(finalPor < 0.05, finalPor50, ifelse(finalPor < 0.1, finalPor20, ifelse(finalPor < 1, finalPor10, finalPor)))), digits = 2), ifelse(finalPor < 0.05, "1:50", ifelse(finalPor < 0.1, "1:20", ifelse(finalPor < 1, "1:10", "No Dilution"))), round(as.numeric(mmVol), digits = 2), round(as.numeric(adjmRNA), digits = 2), round(as.numeric(mmOpti), digits = 2), round(as.numeric(mRNAOpti), digits = 2))
    ...
    output$text1 = renderText({
        ...
        #Repeat function for access in this block
        mRNA_transfections2 = function(volTf, numWells, delWell, porPerc, mmRNARatio, messMaxDil) {
